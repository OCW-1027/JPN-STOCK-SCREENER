...
def load_snapshots(m):
    ...
    for f in files:
        ...
        _x = [c for c in sigcols if dict((a, g) for a, _, g in SIGNALS)[c] == "x"]
        df["grp_supply"] = df[_x].any(axis=1) if _x else False
...
.b.x{background:rgba(255,178,36,.14);color:#ffb224}
