...
UI = {
    "ko": dict(
        ...
        c_inflow="유입배", c_score="점수", c_vwap="VWAP괴리%", c_fturn="유동회전%", c_fpct="유동비율%",
        c_cleared="청산", c_svpct="공매도비중%", c_svchg="비중Δ", c_si="숏잔고(백만)", c_dtc="숏커버일",
        c_frgn_d="외국인연속", c_frgn5="외국인5일", c_frgn_r="외국인지분%",
        c_inst_d="기관연속", c_inst5="기관5일", c_indi5="개인5일", c_mratio="대차배율", c_mdays="신용부담일", c_mbuychg="신용매수Δ", c_spct="공개숏포지션%", c_schg="숏포지션Δ",
        preset_supply="수급", c_peg="PEG", c_ytd="연초대비%", c_hi52="52주고가", c_lo52="52주저가",
        c_pos52="52주위치%", c_perfy="1년%",
        s_growth="고성장", s_accel="이익가속", s_garp="저평가성장", s_inflow="지속유입",
        s_squeeze="숏스퀴즈", s_churn="손바뀜", s_accum="바닥매집", s_distrib="상투분산",
        fresh_title="데이터 기준", fresh_price="시세", fresh_margin="신용잔고(주간)", fresh_short="공개숏포지션",
        fresh_investor="투자자별매매", fresh_finra="공매도비중", tab_sup="수급",
        streak_new="NEW", dropped="이탈", dropped_tip="어제까지 켜져 있다가 오늘 꺼진 시그널 수",
        tab_mine="내 종목", c_star_p="등록가", c_star_r="등록후", star_exp="내보내기", star_imp="가져오기",
        star_imported="{n}종목을 가져왔습니다",
        link_copy="현재 화면 링크", link_copied="복사됨 ✓", nav_macro="📊 마켓 지표",
        ...
        and_off="OR", and_on="AND",
        and_tip="여러 시그널을 켰을 때: OR=하나라도 해당 / AND=전부 해당하는 종목만. 예) 신고가권+지속유입+저평가를 AND로 교차 검색",
        delay="15분 지연 스냅샷",
        delay_tip="장중에는 15분 지연 시세를 15분 간격으로 갱신합니다. 16:30 이후 표시되는 데이터가 당일 종가 기준입니다. 시그널·거래량은 갱신 시점의 값이므로 장중에는 하루치가 덜 반영됩니다.",
        ...
        foot=("단기: <b>급증</b> 거래량 10일 평균 대비 2.5배↑ · <b>5MA/20MA돌파</b> 당일 상향 통과 · "
              ...
              "수급: 시장마다 공개되는 데이터가 달라 표시 항목이 다릅니다 — "
              "<b>일본</b> 대차배율·신용부담일·신용매수Δ·기관공매도(JPX) · "
              "<b>한국</b> 외국인·기관·개인 순매수와 연속일수(네이버) · "
              "<b>미국</b> 공매도비중·숏잔고·숏커버일(FINRA·나스닥)<br>"
    ...
    "ja": dict(
        ...
        c_inflow="流入倍", c_score="スコア", c_vwap="VWAP乖離%", c_fturn="浮動株回転%", c_fpct="浮動株比率%",
        c_cleared="清算", c_svpct="空売り比率%", c_svchg="比率Δ", c_si="ショート残(百万)", c_dtc="カバー日数",
        c_frgn_d="外国人連続", c_frgn5="外国人5日", c_frgn_r="外国人保有%",
        c_inst_d="機関連続", c_inst5="機関5日", c_indi5="個人5日", c_mratio="貸借倍率", c_mdays="信用負担日", c_mbuychg="信用買Δ", c_spct="公開ショート%", c_schg="ショートΔ",
        preset_supply="需給", c_peg="PEG", c_ytd="年初来%", c_hi52="52週高値", c_lo52="52週安値",
        c_pos52="52週位置%", c_perfy="1年%",
        s_growth="高成長", s_accel="利益加速", s_garp="割安成長", s_inflow="継続流入",
        s_squeeze="踏み上げ", s_churn="手替わり", s_accum="底値買い集め", s_distrib="天井分散",
        fresh_title="データ基準", fresh_price="株価", fresh_margin="信用残(週間)", fresh_short="公開ショート",
        fresh_investor="投資家別売買", fresh_finra="空売り比率", tab_sup="需給",
        streak_new="NEW", dropped="離脱", dropped_tip="昨日まで点灯し今日消えたシグナル数",
        tab_mine="マイ銘柄", c_star_p="登録価格", c_star_r="登録後", star_exp="書き出し", star_imp="読み込み",
        star_imported="{n}銘柄を読み込みました",
        link_copy="現在の画面リンク", link_copied="コピー済 ✓", nav_macro="📊 マーケット指標",
        ...
        and_off="OR", and_on="AND",
        and_tip="複数シグナル選択時: OR=いずれか該当 / AND=すべて該当する銘柄のみ。例) 高値圏+継続流入+割安をANDで絞り込み",
        delay="15分遅延スナップショット",
        delay_tip="場中は15分遅延の株価を15分間隔で更新します。16:30以降の表示が当日終値基準です。シグナル・出来高は更新時点の値のため、場中は1日分が未反映です。",
        ...
        foot=("短期: <b>出来高急増</b> 10日平均比2.5倍↑ · <b>5MA/20MA突破</b> 当日上抜け · "
              ...
              "需給: 市場ごとに公開データが異なるため表示項目が変わります — "
              "<b>日本</b> 貸借倍率・信用負担日・信用買Δ・機関空売り(JPX) ・ "
              "<b>韓国</b> 外国人・機関・個人の純買いと連続日数(NAVER) ・ "
              "<b>米国</b> 空売り比率・ショート残・カバー日数(FINRA・NASDAQ)<br>"
...
SIGNAL_I18N = {
    ...
    "sig_squeeze": ("숏스퀴즈", "踏み上げ"), "sig_churn": ("손바뀜", "手替わり"),
    "sig_accum": ("바닥매집", "底値買い集め"), "sig_distrib": ("상투분산", "天井分散"), "grp_supply": ("수급(아무거나)", "需給(いずれか)"),
...
HELP = {
    ...
    "c_vwap": ("종가 − 당일 거래대금가중평균가(VWAP). 양수면 평균 매수단가보다 높게 마감 → 적극 매수 우위. "
               "장중에는 갱신 시점 기준.",
               "終値−当日VWAP。プラスなら平均約定価格より高く引け→積極的な買い優勢。場中は更新時点基準。"),
    "c_fturn": ("당일 거래량 ÷ 유동주식수. 상호보유가 많은 일본에서는 전체 주식 대비보다 실제 손바뀜을 정확히 보여줍니다. "
                "50%↑면 세력 손바뀜 급등 의심.",
                "当日出来高÷浮動株数。持ち合いが多い日本では実際の手替わりをより正確に示します。50%↑は大口の手替わり急増。"),
    "c_fpct": ("발행주식 중 실제 시장 유통 비율. 낮을수록(20~30% 미만) 품절주 성격 — 작은 수급에도 크게 움직입니다.",
               "発行株式のうち実際に流通する比率。低いほど(20〜30%未満)品薄株の性格で、小さな需給でも大きく動きます。"),
    "c_svpct": ("당일 거래량 중 공매도가 차지한 비중 (FINRA 일별 공시). 40~60%도 흔하며 "
                "시장조성자 헤지가 상당수 포함되므로, 절대 수준보다 '평소 대비 급변'을 봐야 합니다.",
                "当日出来高に占める空売りの比率(FINRA日次)。40〜60%も一般的で、マーケットメイカーの"
                "ヘッジを多く含むため、水準より'平時比の急変'を見るべきです。"),
    "c_svchg": ("공매도 비중의 전일 대비 증감(%p). +10%p 이상 급증은 특정 세력의 숏 진입 가능성.",
                "空売り比率の前日比増減(%p)。+10%p以上の急増は特定筋のショート参入の可能性。"),
    "c_si": ("Short Interest 잔고(백만주). 격주 공시라 최대 2주 전 상태입니다. 거래대금 상위 종목만 수집.",
             "ショート残高(百万株)。隔週開示のため最大2週間前の状態。売買代金上位のみ収集。"),
    "c_dtc": ("Days to Cover = 숏 잔고 ÷ 일평균 거래량. 숏 전량 청산에 며칠 걸리는지. "
              "5일 이상이면 숏스퀴즈 시 상승 폭이 커질 수 있습니다.",
              "Days to Cover=ショート残高÷平均出来高。全量buy backに何日かかるか。"
              "5日以上なら踏み上げ時の上昇幅が大きくなり得ます。"),
    "c_frgn_d": ("외국인 연속 순매수 일수. 양수면 연속 매수, 음수면 연속 매도. "
                 "한국 시장에서 외국인의 연속 매수는 추세 지속력이 강한 편입니다.",
                 "外国人の連続純買い日数。プラスは連続買い、マイナスは連続売り。"),
    "c_frgn5": ("최근 5영업일 외국인 순매수 합계(천주). 하루 반짝이 아닌 누적 방향을 봅니다.",
                "直近5営業日の外国人純買い合計(千株)。"),
    "c_frgn_r": ("외국인 보유 지분율. 상승 추세면 지속 매집, 외국인 한도 근처 종목은 수급이 타이트합니다.",
                 "外国人保有比率。上昇基調なら継続的な買い集め。"),
    "c_inst_d": ("기관 연속 순매수 일수. 외국인과 방향이 같으면 신뢰도가 높아집니다.",
                 "機関の連続純買い日数。外国人と同方向なら信頼度が上がります。"),
    "c_inst5": ("최근 5영업일 기관 순매수 합계(천주).", "直近5営業日の機関純買い合計(千株)。"),
    "c_indi5": ("최근 5영업일 개인 순매수 합계(천주). 개인만 사고 외국인·기관이 파는 구간은 주의.",
                "直近5営業日の個人純買い合計(千株)。個人だけが買う局面は注意。"),
    "c_mratio": ("신용매수잔고 ÷ 신용매도잔고 (JPX 주간, 전주 금요일 기준). 1 미만이면 공매도가 더 많아 숏스퀴즈 가능성. "
                 "단, 주주우대 크로스 종목(외식·소매)은 우대 목적 매도잔고라 약세 신호가 아님.",
                 "信用買残÷信用売残(JPX週間、前週金曜基準)。1未満は売り残が多く踏み上げの可能性。"
                 "ただし優待クロス銘柄(外食・小売)は優待目的の売残なので弱気ではない。"),
    "c_mdays": ("신용매수잔고 ÷ 10일 평균 거래량. 물린 물량을 소화하는 데 며칠 걸리는지. 10일↑면 반등마다 매물(戻り売り) 부담.",
                "信用買残÷10日平均出来高。含み損玉の消化に何日かかるか。10日↑は戻り売り圧力が重い。"),
    "c_mbuychg": ("신용매수잔고 전주 대비 증감(주). 급증하면 개인 추격매수 과열, 급감하면 손절·투매.",
                  "信用買残の前週比増減(株)。急増は個人の追随買い過熱、急減は損切り・投げ。"),
    "c_spct": ("JPX 공개 기준(발행주식 0.5%↑)을 넘긴 포지션만 합산한 값. 시장 전체 공매도 잔고가 아니며 "
               "0.5% 미만은 포함되지 않습니다. 비어 있으면 공개 대상 기관이 없다는 뜻.",
               "JPX公開基準(0.5%↑)を超えたポジションのみの合計。市場全体の空売り残高ではなく、0.5%未満は含みません。"),
    "c_schg": ("기관 공매도 잔고 비율의 직전 대비 증감(%p). 마이너스면 숏커버링 진행 중.",
               "機関空売り残高比率の直前比増減(%p)。マイナスはショートカバー進行中。"),
...
HELP_SIG = {
    ...
    "s_squeeze": ("대차배율 0.8 이하 & 신용매도잔고가 10일 거래량의 3배↑ & 종가>20일선. 공매도가 몰린 상태에서 "
                  "주가가 오르면 숏커버 급등이 나올 수 있습니다. ⚠ 외식·소매 종목은 주주우대 크로스일 가능성이 높으니 제외하고 보세요.",
                  "貸借倍率0.8以下かつ信用売残が10日出来高の3倍↑かつ終値>20日線。⚠ 外食・小売は優待つなぎ売りの可能性大。"),
    "s_churn": ("유동주식 회전율 30%↑이면서 상승 마감. 실제 유통 물량의 3분의 1이 하루에 손바뀜한 것 — 세력 진입 신호.",
                "浮動株回転率30%↑かつ上昇引け。流通株の1/3が1日で手替わり、大口参入のサイン。"),
    "s_accum": ("52주 위치 25% 이하(바닥권)인데 큰손이 사는 중. 한국: 외인 3일↑ 연속 순매수 & 기관도 5일 순매수 / 일본: 공개 숏포지션 감소+상승 / 미국: 공매도비중 10%p↑ 급감+상승.",
                "52週位置25%以下(底値圏)で大口が買い集め中。韓国:外国人3日↑連続買いかつ機関も5日純買い/日本:公開ショート減少+上昇/米国:空売り比率10%p↑急減+上昇。"),
    "s_distrib": ("52주 위치 90%↑(천장권)인데 개인만 사고 외인·기관은 2일↑ 연속 순매도. 큰손이 개인에게 넘기는 분산 국면. (한국 데이터)",
                  "52週位置90%↑で個人だけ買い、外国人・機関は2日↑連続売り。大口が個人に渡す分散局面。(韓国データ)"),
